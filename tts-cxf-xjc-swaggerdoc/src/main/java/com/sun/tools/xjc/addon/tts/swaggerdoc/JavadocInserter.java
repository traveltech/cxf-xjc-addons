...
public class JavadocInserter {
    ...
    private void addJavadocs(ClassOutline classOutline) {
        ...
        if (!jDefinedClass.javadoc().isEmpty()) {

            final StringBuffer javaDocs = new StringBuffer();

            for (Object object : jDefinedClass.javadoc()){
                javaDocs.append(object.toString()).append(" ");
            }

            final String  javaDoc = javaDocs.toString();

            int startOfJaxbJavaDoc = javaDoc.indexOf("<p>");

            if (startOfJaxbJavaDoc != -1) {

                try {
                    final Class<?> draftClass = this.getClass().getClassLoader()
                            .loadClass("io.swagger.annotations.ApiModel");
                    ...
                    if (Annotation.class.isAssignableFrom(draftClass)) {
                        ...
                        jDefinedClass.annotate(annotationClass).param("description", javaDoc.substring(0, startOfJaxbJavaDoc).trim());
                    }
